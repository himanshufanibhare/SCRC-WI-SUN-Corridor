...
sl_wisun_coap_packet_t * coap_callback_relay3on (
  const sl_wisun_coap_packet_t *const req_packet){
  sl_simple_led_turn_off(sl_led_led2.context);
  printf("RELAY3 ON !!");
  char *response_json = "{\"Message\": \"RELAY3 ON!\"}";
  return app_coap_reply(response_json, req_packet);
}



sl_wisun_coap_packet_t * coap_callback_relay3off (
  const sl_wisun_coap_packet_t *const req_packet){
  sl_simple_led_turn_on(sl_led_led2.context);
  printf("RELAY3 OFF !!");
  char *response_json = "{\"Message\": \"RELAY3 OFF!\"}";
  return app_coap_reply(response_json, req_packet);
}



sl_wisun_coap_packet_t * coap_callback_relay4on (
  const sl_wisun_coap_packet_t *const req_packet){
  sl_simple_led_turn_off(sl_led_led3.context);
  printf("RELAY4 ON !!");
  char *response_json = "{\"Message\": \"RELAY4 ON!\"}";
  return app_coap_reply(response_json, req_packet);
}



sl_wisun_coap_packet_t * coap_callback_relay4off (
  const sl_wisun_coap_packet_t *const req_packet){
  sl_simple_led_turn_on(sl_led_led3.context);
  printf("RELAY4 OFF !!");
  char *response_json = "{\"Message\": \"RELAY4 OFF!\"}";
  return app_coap_reply(response_json, req_packet);
}
...
sl_wisun_coap_packet_t * coap_callback_relay1state (
  const sl_wisun_coap_packet_t *const req_packet){
  sl_led_state_t led_state = sl_simple_led_get_state(sl_led_led0.context);
  if (led_state){
...
sl_wisun_coap_packet_t * coap_callback_relay2state (
  const sl_wisun_coap_packet_t *const req_packet){
  led_state = sl_simple_led_get_state(sl_led_led1.context);
  if (led_state){
...
sl_wisun_coap_packet_t * coap_callback_relay3state (
  const sl_wisun_coap_packet_t *const req_packet){
  led_state = sl_simple_led_get_state(sl_led_led2.context);
  if (led_state){
      printf("RELAY2 STATE : ON ");
      sprintf(response_json, "{\"Message\": \"RELAY3 state: ON\"}");
  } else {
      printf("RELAY2 STATE : OFF ");
      sprintf(response_json, "{\"Message\": \"RELAY3 state: OFF\"}");
  }
  return app_coap_reply(response_json, req_packet);
}

sl_wisun_coap_packet_t * coap_callback_relay4state (
  const sl_wisun_coap_packet_t *const req_packet){
  led_state = sl_simple_led_get_state(sl_led_led3.context);
  if (led_state){
      printf("RELAY4 STATE : ON ");
      sprintf(response_json, "{\"Message\": \"RELAY4 state: ON\"}");
  } else {
      printf("RELAY2 STATE : OFF ");
      sprintf(response_json, "{\"Message\": \"RELAY4 state: OFF\"}");
  }
  return app_coap_reply(response_json, req_packet);
}
...
uint8_t app_coap_resources_init() {
  ...
  coap_resource.data.uri_path = "/relay3on";
  coap_resource.data.resource_type = "json";
  coap_resource.data.interface = "control";
  coap_resource.auto_response = coap_callback_relay3on;
  coap_resource.discoverable = true;
  assert(sl_wisun_coap_rhnd_resource_add(&coap_resource) == SL_STATUS_OK);
  count++;

  coap_resource.data.uri_path = "/relay3off";
  coap_resource.data.resource_type = "json";
  coap_resource.data.interface = "control";
  coap_resource.auto_response = coap_callback_relay3off;
  coap_resource.discoverable = true;
  assert(sl_wisun_coap_rhnd_resource_add(&coap_resource) == SL_STATUS_OK);
  count++;


  coap_resource.data.uri_path = "/relay4on";
  coap_resource.data.resource_type = "json";
  coap_resource.data.interface = "control";
  coap_resource.auto_response = coap_callback_relay4on;
  coap_resource.discoverable = true;
  assert(sl_wisun_coap_rhnd_resource_add(&coap_resource) == SL_STATUS_OK);
  count++;

  coap_resource.data.uri_path = "/relay4off";
  coap_resource.data.resource_type = "json";
  coap_resource.data.interface = "control";
  coap_resource.auto_response = coap_callback_relay4off;
  coap_resource.discoverable = true;
  assert(sl_wisun_coap_rhnd_resource_add(&coap_resource) == SL_STATUS_OK);
  count++;
...
//  count++;
//
//  coap_resource.data.uri_path = "/relay3state";
//  coap_resource.data.resource_type = "json";
//  coap_resource.data.interface = "control";
//  coap_resource.auto_response = coap_callback_relay3state;
//  coap_resource.discoverable = true;
//  assert(sl_wisun_coap_rhnd_resource_add(&coap_resource) == SL_STATUS_OK);
//  count++;
//
//  coap_resource.data.uri_path = "/relay4state";
//  coap_resource.data.resource_type = "json";
//  coap_resource.data.interface = "control";
//  coap_resource.auto_response = coap_callback_relay4state;
//  coap_resource.discoverable = true;
//  assert(sl_wisun_coap_rhnd_resource_add(&coap_resource) == SL_STATUS_OK);
//  count++;
//
